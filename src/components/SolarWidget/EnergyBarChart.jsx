...
const EnergyBarChart = memo(({ data, showBattery = true }) => {
        ...
        const result = [
            {
                ...
                unit: 'kW',
                color: data.grid < 0 ? COLORS.gridExport : COLORS.gridImport,
                icon: Zap,
    ...
    return (
        <div className="w-full h-full flex flex-col px-2 py-1">
            ...
            <div className={`relative flex-1 flex items-stretch ${showBattery ? 'justify-center gap-3' : 'justify-center gap-4'}`}>
                {bars.map((bar) => {
                    const Icon = bar.icon;
                    const rawPercent = bar.id === 'battery' ? bar.value : (bar.value / maxValue) * 100;
                    // Cap at 70% to avoid being "too long" and maintain 15% minimum
                    const heightPercent = Math.min(Math.max(rawPercent, 15), 70);
